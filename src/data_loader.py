...
def afficher_exemples(chemins: list, labels: list,
                      n: int = 8,
                      save_path: str = "metrics/exemples_images.png",
                      seed: int = 42) -> None:
    ...
    selectionnes  = rng.sample(idx_parasit,  n // 2)
    selectionnes += rng.sample(idx_uninfect, n // 2)
    rng.shuffle(selectionnes)
...
def stats_dataset(chemins: list, labels: list, seed: int = 42) -> None:
    ...
    echantillon = random.Random(seed).sample(chemins, min(20, len(chemins)))
