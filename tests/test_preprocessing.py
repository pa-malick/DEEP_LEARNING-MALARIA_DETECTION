...
from src.preprocessing import split_donnees


@pytest.fixture
def donnees_fictives():
    """Génère 200 chemins et labels fictifs (100 par classe)."""
    chemins = [f"/fake/Parasitized/img_{i}.png" for i in range(100)] + \
...
class TestSplitDonnees:
    ...
    def test_tailles_coherentes(self, donnees_fictives):
        ...
        total = len(df_tr) + len(df_val) + len(df_te)
        assert total == len(chemins), "La somme des splits doit égaler le total"
    ...
    def test_pas_de_chevauchement(self, donnees_fictives):
        """Les ensembles train, val et test ne doivent pas partager de chemins."""
        chemins, labels = donnees_fictives
        df_tr, df_val, df_te = split_donnees(chemins, labels)
